...
class GTSensor:
	...
	def writePacket(self, cmd, param):
		packet = bytearray(struct.pack(comm_struct(), 0x55, 0xAA, self.address, param, cmd))
		checksum = sum(packet)
	...
	def decode_command(self, rxPacket):
		...
		checksum = sum(struct.unpack(CHECKSUM_STRUCT(), rxPacket[-2:]))
		rxPacket = rxPacket[:-2]
	...
	def decode_data(self, rxPacket):
		...
		if rxPacket[0] == GT521F5.CMD_DATA_1.value and GT521F5.CMD_DATA_2.value:
	...
	def encode_data(self, data, length, address):
		...
		txPacket += bytearray(struct.pack(GT521F5.CHECKSUM_STRUCT(), checksum))
		return txPacket
	...
	def captureFinger(self, hd = False):
		...
		if self.writePacket(GT521F5.CAPTURE_IMAGE.value, hx*1)
			self.serial.timeout = self.usb_timeout
			return self.receivedPacket()
		else:
			raise RuntimeError("Couldn't send packet.")
